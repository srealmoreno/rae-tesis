...
function get_distro() {
	...
	if [ "$ID" != 'ubuntu' ] && [ "$ID_LIKE" != 'ubuntu' ]; then
		msg_error "Unable to detect your distro '$ID' not supported. Only Ubuntu and its derivatives are supported"
	fi
	...
	if [ "$CODENAME" == '' ]; then
		msg_error "Unable to detect your distro '$ID' not supported. 'Codename' is not defined"
	fi
...
function add_gns3_repo() {
	msg_info 'Adding GNS3 repository'
